  ...
  useEffect(() => {
    ...
    class MazeScene extends Phaser.Scene {
      ...
      createMobileControls() {
        const controlSize = this.tileSize * 1.5;
        const padding = this.tileSize;
        
        // D-pad container
        const dpad = this.add.container(
          padding + controlSize * 1.5,
          dimensions.height - padding - controlSize * 1.5
        );

        // Create direction buttons
        const up = this.add.rectangle(0, -controlSize, controlSize, controlSize, 0x29484e, 0.7)
          .setInteractive()
          .on('pointerdown', () => { activeDirection = 'up'; this.startMovement(); })
          .on('pointerup', () => { activeDirection = null; this.stopMovement(); })
          .on('pointerout', () => { activeDirection = null; this.stopMovement(); });

        const down = this.add.rectangle(0, controlSize, controlSize, controlSize, 0x29484e, 0.7)
          .setInteractive()
          .on('pointerdown', () => { activeDirection = 'down'; this.startMovement(); })
          .on('pointerup', () => { activeDirection = null; this.stopMovement(); })
          .on('pointerout', () => { activeDirection = null; this.stopMovement(); });

        const left = this.add.rectangle(-controlSize, 0, controlSize, controlSize, 0x29484e, 0.7)
          .setInteractive()
          .on('pointerdown', () => { activeDirection = 'left'; this.startMovement(); })
          .on('pointerup', () => { activeDirection = null; this.stopMovement(); })
          .on('pointerout', () => { activeDirection = null; this.stopMovement(); });

        const right = this.add.rectangle(controlSize, 0, controlSize, controlSize, 0x29484e, 0.7)
          .setInteractive()
          .on('pointerdown', () => { activeDirection = 'right'; this.startMovement(); })
          .on('pointerup', () => { activeDirection = null; this.stopMovement(); })
          .on('pointerout', () => { activeDirection = null; this.stopMovement(); });

        // Add direction arrows
        const arrowStyle = { font: `${controlSize * 0.6}px Arial`, fill: '#ffffff' };
        this.add.text(0, -controlSize, '↑', arrowStyle).setOrigin(0.5);
        this.add.text(0, controlSize, '↓', arrowStyle).setOrigin(0.5);
        this.add.text(-controlSize, 0, '←', arrowStyle).setOrigin(0.5);
        this.add.text(controlSize, 0, '→', arrowStyle).setOrigin(0.5);

        dpad.add([up, down, left, right]);
      }

      startMovement() {
        if (movementInterval) clearInterval(movementInterval);
        movementInterval = setInterval(() => this.handleMovement(), 16);
      }
      ...
      handleMovement() {
        ...
        switch (activeDirection) {
          case 'up': this.player.setVelocityY(-speed); break;
          case 'down': this.player.setVelocityY(speed); break;
          case 'left': this.player.setVelocityX(-speed); break;
          case 'right': this.player.setVelocityX(speed); break;
          default: this.player.setVelocity(0);
        }
      }
